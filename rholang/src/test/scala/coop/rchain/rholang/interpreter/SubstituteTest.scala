...
class SendSubSpec extends FlatSpec with Matchers {
  ...
  "Send" should "substitute all bound vars for values" in {
    ...
    val target = Send(
      ...
      data = List(Send(EVar(BoundVar(0)), List(Par()), false, BitSet(0))),
      false,
      BitSet(0)
    )
    val _target =
...
class NewSubSpec extends FlatSpec with Matchers {
  ...
  "New" should "only substitute body of expression" in {
    ...
    result should be(
      New(bindCount = 1, p = Send(source, List(Par()), false, BitSet()), Vector.empty, BitSet())
    )
  }
